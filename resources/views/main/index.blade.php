<x-home.container class="h-screen flex items-center">
    <div class="left-10 absolute ml-[50%] translate-x-[-50%]">
        @if($quote->count()) @foreach($quote as $curQuote) @endforeach @php
        foreach($movies as $currentMovie) @endphp
        <img
            src="{{asset('storage/' . $curQuote->image)}}"
            alt=""
            class="w-[700px] h-[386px] rounded-[10px]"
        />
        ...
        <p class="text-center text-white text-5xl underline mt-28">
            <a
                href="{{route('current_movie', ['locale' => Config::get('app.locale'), 'id' => $currentMovie->id])}}"
            >
                @if(Config::get('app.locale') == 'en')
